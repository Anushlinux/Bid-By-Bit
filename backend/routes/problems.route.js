  ...
  for (let i = 0; i < 10; i++) {
    ...
    for (let j = 0; j < tasks.length; j++) {
      ...
      if (
        ...
        runtime +=
          new Date(status.data.execution[j].completedAt) -
          new Date(status.data.execution[j].startedAt);
      } else {
        d["successful"] = false;
        d["error"] = status.data.execution[j].error;

        runtime +=
          new Date(status.data.execution[j].failedAt) -
          new Date(status.data.execution[j].startedAt);
      }
      d["hidden"] = problem.testCases[j].hidden;
      if (!problem.testCases[j].hidden) {
    ...
    console.log("DATA:", { data, runtime, accepted, submission: true });
    return res.status(200).json({ data, runtime, accepted, submission: true });
  }
...
router.post("/:id/run", async (req, res) => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    return res.status(200).json({ data, runtime, submission: false });
  }
